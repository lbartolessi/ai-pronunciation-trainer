...
for lang in available_languages:
    df = pd.read_csv(SAMPLE_FOLDER + "data_" + lang + ".csv", delimiter=";")
    lambda_database[lang] = TextDataset(df)
    lambda_ipa_converter[lang] = rule_based_models.get_phonem_converter(lang)
...
def lambda_handler(event):
    ...
    while not sample_in_category:
        ...
        sentence_category = get_sentence_category(current_transcript[0])

        sample_in_category = (sentence_category == category) or category == 0

    translated_trascript = ""

    current_ipa = lambda_ipa_converter[language].convert_to_phonem(current_transcript[0])
...
def get_sentence_category(sentence) -> int:
    ...
    for category in range(len(categories_word_limits) - 1):
        if (
            number_of_words > categories_word_limits[category]
            and number_of_words <= categories_word_limits[category + 1]
        ):
            return category + 1
